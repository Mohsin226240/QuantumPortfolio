  ...
  return (
    <motion.div layout className={`rounded-xl border overflow-hidden transition-colors duration-200 ${open ? "bg-emerald-50 border-emerald-200" : "bg-white border-emerald-100"}`}>
      <button onClick={() => setOpen(o => !o)} className="w-full flex items-center justify-between px-5 py-4 text-left group">
        ...
        <motion.div animate={{ rotate: open ? 180 : 0 }} transition={{ duration: 0.3, ease: [0.25, 0.1, 0.25, 1] }}
          className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ml-3 transition-all duration-300 ${open ? "bg-emerald-500 shadow-md shadow-emerald-500/25" : "bg-emerald-50"}`}>
          <svg width="14" height="14" viewBox="0 0 12 12" fill="none">
            <path d="M2.5 4.5L6 8L9.5 4.5" stroke={open ? "#fff" : "#10b981"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
        </motion.div>
      </button>
      <AnimatePresence>
...
export default function FAQSection({ scrollTo }) {
  return (
    <section id="faq" className="py-14 sm:py-20 lg:py-24 px-4 sm:px-6 lg:px-8 bg-white transition-colors duration-500">
      <div className="max-w-[1320px] mx-auto">
        <div className="grid lg:grid-cols-2 gap-8 sm:gap-12 lg:gap-16 items-start">
          <motion.div initial={{ opacity: 0, x: -80, y: 60 }} whileInView={{ opacity: 1, x: 0, y: 0 }} transition={{ duration: 3.0, ease: [0.16, 1, 0.3, 1] }} viewport={{ once: true, margin: "-60px" }} className="lg:sticky lg:top-28">
            <h2 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-slate-900 mb-3 sm:mb-4">Frequently<br /><span style={{ background: "linear-gradient(135deg, #10b981, #14b8a6, #06b6d4)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Asked Questions</span></h2>
            <p className="text-sm sm:text-base text-slate-500 mb-6 sm:mb-8 leading-relaxed max-w-md">Elite Fusion Trading is gaining popularity in the crypto and forex space. Effective account management and understanding market fundamentals are crucial for success.</p>
          ...
          <motion.div initial={{ opacity: 0, x: 80, y: 60 }} whileInView={{ opacity: 1, x: 0, y: 0 }} transition={{ duration: 3.0, ease: [0.16, 1, 0.3, 1] }} viewport={{ once: true, margin: "-60px" }} className="space-y-3">
            {faqs.map((faq, i) => (
